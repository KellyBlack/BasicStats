...
\iftoggle{clicker}{%
  ...
  \begin{frame}
    ...
    I have eight markers, and they are all different colours. I pull
    ...
    red one what is the greatest number of colours that I can use
    today?
...
\begin{frame}
  ...
  \begin{eqnarray*}
    {P}_{5}^{40} & = & 40\cdot 39 \cdot 38 \cdot 37 \cdot 36, \\
    {P}_{40}^{40} & = & \frac{40!}{0!}, \\
    {P}_2^{40} & = & 40\cdot 39 ~ = ~ \frac{40!}{38!}. \\
  \end{eqnarray*}
...
\begin{frame}
  ...
  \only<2->%
  {
    ...
    There are ${P}_k^{n}$ permutations. Each permutation
    can be expressed in $k!$ different ways,
    \begin{eqnarray*}
      {P}_k^{n} & = & k! {C}_k^{n}, \\
      {C}_k^{n} & = & \frac{{P}_k^{n}}{k!}, \\
      {C}_k^{n} & = & \frac{n!}{(n-k)!k!}.
    \end{eqnarray*}
  }
